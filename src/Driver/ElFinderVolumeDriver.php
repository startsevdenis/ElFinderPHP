...
abstract class ElFinderVolumeDriver
{
	...
	protected function stat($path)
	{
		...
		return isset($this->cache[$path]) ? $this->cache[$path] : $this->updateCache($path, $this->convEncOut($this->_stat($this->convEncIn($path))));
	}
	...
	protected function updateCache($path, $stat)
	{
		...
		elseif ($this->attr($path, 'hidden', !empty($stat['hidden']), $isDir) || !$this->mimeAccepted($stat['mime']))
		{
		...
		{
			$stat['url'] = rtrim($this->options['URL'], '/') . '/' . str_replace('%2F', '/', rawurlencode(str_replace($this->root, "", $this->convEncIn($path))));
		}
	...
	protected function copy($src, $dst, $name)
	{
		...
		if (!empty($srcStat['thash']))
		{
			...
			$this->clearcache();
			return $stat && $this->symlinkCE($target, $dst, $name) ? $this->joinPathCE($dst, $name) : $this->setError(elFinder::ERROR_COPY, $this->path($src));
		}
		...
		return $this->_copy($this->convEncIn($src), $this->convEncIn($dst), $this->convEncIn($name)) ? $this->joinPathCE($dst, $name) : $this->setError(elFinder::ERROR_COPY, $this->path($src));
	}
